...
public extension Sequence where Self: RangeReplaceableCollection, Self: RandomAccessCollection, Iterator.Element == Proposal {
  ...
  func filter(author: Person) -> [Proposal] {
    ...
    guard author.name.isEmpty == false else {
      return []
    }
    ...
    let authors = self.filter {
      ...
      return authors.filter(by: author.name).isEmpty == false
    }
  ...
  func filter(manager: Person) -> [Proposal] {
    ...
    guard managerName.isEmpty == false else {
      return []
    }

    let reviewManagers = self.filter {
      guard
        let manager = $0.reviewManagers?.first,
  ...
  func filter(by value: String) -> [Proposal] {
    ...
    let withPrefixes = self.filter { String($0.identifierFormatted).lowercased() == value.lowercased() }
    ...
    // Status
    let statuses = self.filter { $0.status.state.name.contains(value) }
    ...
    let reviews = self.filter {
      ...
      return manager.name.contains(value) || username == value
    }
