...
public extension Sequence where Self: RangeReplaceableCollection, Self: RandomAccessCollection, Iterator.Element == Person {
  func filter(by value: String) -> [Person] {
    var filter: [Person] = []

    // Names
    let names = self.filter { $0.name.contains(value) }
  ...
  func get(name: String) -> Person? {
    guard
      let index = firstIndex(where: { $0.name == name })
    else {
      return nil
    }
